...
export const StrategyHistory = () => {
 ...
 const itemDate = item.started_at ? new Date(item.started_at).toLocaleDateString('en-CA') : ''; // en-CA gives YYYY-MM-DD
 ...
 {/* Header Controls */}
 <div className="flex flex-col md:flex-row gap-3 items-center justify-between bg-white p-2.5 rounded-lg border border-slate-100">
 ...
 <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
  <div className="flex items-center bg-slate-50 rounded-lg p-0.5 w-full sm:w-auto">
  <input 
  type="date" 
  value={dateFilter}
  onChange={(e) => setDateFilter(e.target.value)}
  className="w-full sm:w-auto min-w-[130px] px-2.5 py-1 bg-transparent border-none rounded-md text-[11px] font-bold text-slate-700 outline-none cursor-pointer"
  />
  {dateFilter && (
  <button 
  onClick={() => setDateFilter('')} 
  className="p-1 mr-0.5 rounded-md text-slate-400 hover:text-red-500 hover:bg-white transition-colors"
  title="Clear date"
  >
 ...
 <div className="flex items-center gap-1.5 bg-slate-50 p-0.5 rounded-lg w-full sm:w-auto">
 {[
 ...
 className={`flex-1 sm:flex-none px-3 py-1 rounded-md text-[10px] font-bold transition-all ${filterType === t.id
 ...
 <span className={`px-1 rounded-[4px] uppercase font-black tracking-tighter scale-90 ${item.status ==='COMPLETED' ?'bg-emerald-50 text-emerald-600' :
 ...
 <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded border ${item.config?.is_paper_trading
 ...
 <div key={idx} className="bg-white border border-slate-200/60 rounded-lg p-2.5 flex flex-col gap-2">
 {/* Header Info */}
