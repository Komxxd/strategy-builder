...
function pauseStrategy(strategyId, reason) {
    ...
    marketSocketService.sendAlertToUser(strategy.user_id, `Strategy PAUSED — ${reason}`, "error", strategyId);
}
...
function stopStrategy(strategyId, reason) {
    ...
    const marketSocketService = require("../marketSocket.service");
    marketSocketService.sendAlertToUser(strategy.user_id, `Strategy CLOSED — ${reason}`, "error", strategyId);
}
...
async function resumeStrategy(strategyId, userId) {
    ...
    marketSocketService.sendAlertToUser(strategy.user_id, `Strategy resumed — monitoring active`, "success", strategyId);
    updateStrategyInMemory(strategyId, {
...
async function switchVirtualMode(strategyId, targetVirtual, userId) {
    ...
    if (targetVirtual) {
            ...
            if (!leg.exited) {
                ...
                if (leg.entryPrice) {
                    ...
                    const virtualLeg = {
                        ...originalLeg,
                        leg: { ...originalLeg.leg },
                        instrument: originalLeg.instrument ? { ...originalLeg.instrument } : null,
                        orderId: `VIRTUAL-${Date.now()}-${originalLeg.legIndex}`,
                        ...
                        exitType: null,
                        isExiting: false,
                        entryTime: originalLeg.entryTime || exitTime,
                        entryPrice: originalLeg.entryPrice, // Continuous entry price for "what-if" PnL
                        currentLtp: exitLtp,
                    ...
                    const oldState = originalLeg.state;
                    ...
                    const virtualLeg = {
                        ...
                        exitType: null,
                        isExiting: false,
                        entryTime: oldState === "ACTIVE" ? exitTime : originalLeg.entryTime,
                        entryPrice: oldState === "ACTIVE" ? originalLeg.currentLtp : originalLeg.entryPrice,
                        slOrderId: null,
                        slUniqueOrderId: null,
                        exitOrderId: null,
                        bookedPnlPoints: 0,
        ...
        marketSocketService.sendAlertToUser(strategy.user_id, `Strategy switched to VIRTUAL mode — active positions closed, virtual monitoring running`, "info", strategyId);
        return true;
    } else {
            ...
            if (!leg.exited) {
                ...
                if (leg.entryPrice) {
                    ...
                    const newRunningLeg = {
                        ...
                        orderId: liveOrderId || `PAPER-${Date.now()}-${originalLeg.legIndex}`,
                        uniqueOrderId: liveUniqueOrderId || `UPAPER-${Date.now()}-${originalLeg.legIndex}`,
                        state: "ACTIVE",
                        ...
                        initialSlTriggerPrice: originalLeg.initialSlTriggerPrice || existingSlTrigger || slTriggerPrice,
                        slLimitPrice: slLimitPrice,
                        peakPrice: Math.max(executionEntryPrice, originalLeg.peakPrice || 0),
                        max_peak_price: Math.max(executionEntryPrice, originalLeg.max_peak_price || 0),
                        max_low_price: originalLeg.max_low_price ? Math.min(executionEntryPrice, originalLeg.max_low_price) : executionEntryPrice,
        ...
        marketSocketService.sendAlertToUser(strategy.user_id, `Strategy switched back to active mode`, "success", strategyId);
        return true;
    }
}
